...
type Tool = "pencil" | "brush" | "eraser" | "bucket";
...
export default function ColoringCanvas({ open, onClose, pages = [], lang = "en", accent = "#1e6fd9" }: ColoringCanvasProps) {
  ...
  const lineDataRef = useRef<Uint8ClampedArray | null>(null); // alpha of line art = fill boundaries
  ...
  // Load the active page's line art into an offscreen canvas so the bucket knows the outlines (boundaries)
  useEffect(() => {
    if (!open) return;
    if (!activePage) { lineDataRef.current = null; return; }
    const img = new Image();
    ...
    img.onload = () => {
      ...
      try { lineDataRef.current = octx.getImageData(0, 0, CW, CH).data; }
      catch { lineDataRef.current = null; }
    };
    img.onerror = () => { lineDataRef.current = null; };
    img.src = activePage;
  }, [activePage, open]);
  ...
  // Paint bucket: flood-fill the tapped region, stopping at the black outlines
  const floodFill = (sx: number, sy: number) => {
    ...
    const tol = 40;
    const isLine = (i: number) => !!line && line[i * 4 + 3] > 100;
    const matches = (i: number) => {
      if (isLine(i)) return false;
      const p = i * 4;
      if (data[p] === fr && data[p + 1] === fg && data[p + 2] === fb) return false;
      return Math.abs(data[p] - tr) <= tol && Math.abs(data[p + 1] - tg) <= tol && Math.abs(data[p + 2] - tb) <= tol;
    };
    if (isLine(sy * CW + sx)) return;             // tapped on a line
    ...
    while (stack.length) {
      ...
      let x = idx % CW;
      const y = (idx - x) / CW;
      while (x >= 0 && matches(y * CW + x)) x--;
      x++;
      let up = false, down = false;
      while (x < CW && matches(y * CW + x)) {
        const p = (y * CW + x) * 4;
        data[p] = fr; data[p + 1] = fg; data[p + 2] = fb; data[p + 3] = 255;
        if (y > 0 && matches((y - 1) * CW + x)) { if (!up) { stack.push((y - 1) * CW + x); up = true; } } else up = false;
        if (y < CH - 1 && matches((y + 1) * CW + x)) { if (!down) { stack.push((y + 1) * CW + x); down = true; } } else down = false;
        x++;
      }
    }
      ...
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-full overflow-y-auto shadow-2xl">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {([
              ["bucket", "🪣", T("Fill", "Rellenar", "Remplir", "Preencher")],
