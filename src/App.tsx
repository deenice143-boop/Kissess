...
export default function App() {
  ...
  // Tap-to-translate language learning
  const [langLearnMode, setLangLearnMode] = useState<boolean>(false);
  const [langLearnTarget, setLangLearnTarget] = useState<string>("es");
  const [activeWordIndex, setActiveWordIndex] = useState<number | null>(null);
  const [activeWordTranslation, setActiveWordTranslation] = useState<string>("");
  const [translatingWord, setTranslatingWord] = useState<boolean>(false);

  const handleWordTap = async (rawToken: string, context: string, index: number) => {
    const word = rawToken.replace(/[^\p{L}\p{N}'-]/gu, "");
    if (!word) return;
    if (activeWordIndex === index) { setActiveWordIndex(null); return; }
    ...
    try {
      const res = await fetch("/api/translate-word", {
        ...
        body: JSON.stringify({ word, context: context.slice(0, 240), targetLanguage: langLearnTarget }),
      });
      const data = await res.json();
      setActiveWordTranslation(data.translation || word);
    } catch {
      setActiveWordTranslation(word);
    } finally {
  ...
  // Theme & Layout States
  const [themeMode, setThemeMode] = useState<"charcoal" | "light">("light");
  ...
  const [paymentHistory, setPaymentHistory] = useState<{ id: string; date: string; amount: string; status: string; plan: string }[]>([
    { id: "TX-4929", date: "2026-06-22", amount: "$9.99", status: "Completed", plan: "KISSS Gold" },
    { id: "TX-2931", date: "2026-05-15", amount: "$9.99", status: "Completed", plan: "KISSS Gold" }
  ]);
  ...
  const handleGenerateLesson = async () => {
    ...
    if (subStatus === "free" && customTopic.trim().length > 0) {
      ...
      if (customCount >= 2) {
        setErrorMessage(
          lang === "es"
            ? "✨ Límite Gratuito Alcanzado: Has generado tus 2 temas de prueba de forma gratuita. ¡Actualiza a KISSS Gold para un sinfín de temas y personajes mágicos!"
            : lang === "fr"
            ? "✨ Limite gratuite atteinte : Vous avez déjà généré vos 2 sujets personnalisés de test. Passez à la formule KISSS Gold pour créer des cours illimités !"
            : "✨ Free limit reached! You've generated your 2 test custom topics. Upgrade to KISSS Gold or Supernova Family for unlimited custom topics and special characters!"
        );
  ...
  return (
    <div className={`min-h-screen flex flex-col selection:bg-indigo-100 selection:text-indigo-900 ${themeMode === "charcoal" ? "theme-charcoal bg-[#0b0f19] text-slate-100" : "bg-[#e6f2ff] text-slate-800"}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center bg-slate-100 rounded-full p-1 border border-slate-200">
              ...
              <button 
                onClick={() => setLang("pt")}
                className={`px-3 py-1 text-xs font-semibold rounded-full transition-all ${lang === "pt" ? "bg-white text-slate-950 shadow-sm" : "text-slate-600 hover:text-slate-900"}`}
              >
                🇧🇷 PT
              </button>
            </div>
        ...
        {currentView === "landing" ? (
          <div className="space-y-12 py-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center max-w-7xl mx-auto pt-6 px-4">
              ...
              >
                <div className="inline-flex items-center gap-2 bg-indigo-600 border border-indigo-600 text-white px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider shadow-sm">
                        ...
                        <div className="space-y-2 flex-1 flex flex-col justify-between">
                          <div className="space-y-1.5">
                            <div className="p-2 bg-blue-50 border border-blue-100 rounded-xl space-y-1">
                              <span className="text-[9px] font-black uppercase text-blue-600 block">{lang === "es" ? "Lección de Cuento Activa" : lang === "fr" ? "Leçon d'Histoire Active" : lang === "pt" ? "Lição em História Ativa" : "Active Story Lesson"}</span>
                              <h5 className="font-extrabold text-slate-900 leading-tight">{lang === "es" ? "La Aventura de División de Leo" : lang === "fr" ? "L'Aventure de Division de Léo" : lang === "pt" ? "A Aventura de Divisão do Léo" : "Leon's Baking Division Adventure"}</h5>
                              <p className="text-[10px] text-slate-500 leading-relaxed">
                                {lang === "es" ? "\"Tenemos 10 chispas de chocolate para repartir en partes iguales entre 3 galletas. ¿Cuántas chispas sobran?\"" : lang === "fr" ? "\"Nous avons 10 pépites de chocolat à partager également entre 3 biscuits. Combien en reste-t-il ?\"" : lang === "pt" ? "\"Temos 10 gotas de chocolate para dividir igualmente entre 3 biscoitos. Quantas gotas sobram?\"" : "\"We have 10 chocolate chips and want to divide them equally between 3 cookies. How many leftover chips are left?\""}
                              </p>
                            </div>

                            <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block">{lang === "es" ? "Elige una opción:" : lang === "fr" ? "Choisis une option :" : lang === "pt" ? "Escolha uma opção:" : "Choose option:"}</span>
                            ...
                            <div className="space-y-1">
                              ...
                              >
                                <span>{lang === "es" ? "A. 0 chispas (Sin sobras)" : lang === "fr" ? "A. 0 pépite (Aucun reste)" : lang === "pt" ? "A. 0 gotas (Sem sobra)" : "A. 0 chips (No remainder)"}</span>
                              ...
                              >
                                <span>{lang === "es" ? "B. ¡Solo sobra 1 chispa!" : lang === "fr" ? "B. Il ne reste qu'1 pépite !" : lang === "pt" ? "B. Sobra apenas 1 gota!" : "B. Only 1 chip left over!"}</span>
                              ...
                              >
                                <span>{lang === "es" ? "C. Sobran 3 chispas" : lang === "fr" ? "C. Il reste 3 pépites" : lang === "pt" ? "C. Sobram 3 gotas" : "C. 3 leftover chips"}</span>
                          ...
                          {simulatedInputOption === 1 && (
                            <div className="p-1.5 bg-emerald-50 border border-emerald-100 rounded-xl text-center text-[10px] text-emerald-800 font-extrabold animate-pulse">
                              {lang === "es" ? "🎉 ¡Genial! ¡Estrella añadida al panel de los padres!" : lang === "fr" ? "🎉 Super ! Étoile ajoutée au tableau des parents !" : lang === "pt" ? "🎉 Incrível! Estrela adicionada ao painel dos pais!" : "🎉 Awesome! Star added to parent scoreboard!"}
                            </div>
                          )}

                          <div className="text-[9px] text-slate-400 text-center italic">
                            {lang === "es" ? "¡Toca la opción B para probar la respuesta correcta!" : lang === "fr" ? "Touche l'option B pour tester la bonne réponse !" : lang === "pt" ? "Toque na opção B para testar a resposta correta!" : "Click option B to test correct answer trigger!"}
                          </div>
                        </div>
                      )}
            ...
            >
              ...
              <div className="space-y-3 text-slate-600 text-base leading-relaxed font-medium text-left sm:text-center">
                <p>{t.missionP1}</p>
                <p>{t.missionP2}</p>
                <p>{t.missionP3}</p>
                <p>{t.missionP4}</p>
              </div>
              <p className="text-lg font-extrabold text-indigo-700 pt-2">{t.missionClose}</p>
            </motion.div>
                ...
                <div className={`bg-gradient-to-b from-white to-amber-50/10 rounded-3xl p-6 border-2 transition-all flex flex-col justify-between relative ${
                  ...
                  <div>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h4 className="text-lg font-black text-slate-900 mt-1">
                          {lang === "es" ? "KISSS Gold Pro" : lang === "fr" ? "Formule Gold Pro" : "KISSS Gold Pro"}
                        </h4>
                      </div>
                      {subStatus === "premium" && (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      alert(lang === "es" 
                        ? "🎉 ¡Actualizado al plan KISSS Gold Pro! Recibes generación ilimitada de temas personalizados y desbloqueas avatars premium." 
                        : lang === "fr"
                        ? "🎉 Bienvenue dans la formule Gold ! Accès illimité aux sujets personnalisés activé."
                        : "🎉 Successfully updated to KISSS Gold Pro (Simulated checkout completion)! Infinite customized AI lessons and premium characters are now unlocked!");
                    }}
                    className={`w-full mt-6 py-2.5 rounded-xl text-xs font-bold transition-all ${
                      subStatus === "premium"
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-xs space-y-4">
                ...
                {activeBook?.downloadUrl && (
                  <a
                    ...
                    download
                    className="mt-4 flex items-center justify-center gap-2 w-full px-4 py-3 rounded-2xl font-bold text-white text-sm shadow-md transition-transform active:scale-95"
                    ...
                    >
                      ...
                      <div className="p-6 sm:p-8 space-y-6">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500">
                                  {isSpeaking 
                                    ...
                                    : (lang === "es" ? "Haz clic para escuchar el cuento narrado" : lang === "fr" ? "Clique pour écouter l'histoire racontée" : lang === "pt" ? "Clique para ouvir a história narrada" : "Click to hear the story narrated")
                                  }
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            {!isSpeaking ? (
                              ...
                              >
                                ...
                                <span>{lang === "es" ? "Escuchar" : lang === "fr" ? "Écouter" : lang === "pt" ? "Ouvir História" : "Hear Story"}</span>
                              </button>
                            ) : (
                              <>
                                {isPaused ? (
                            ...
                            <div className="ml-auto flex items-center gap-1 bg-white border border-slate-100 rounded-xl p-1 shrink-0">
                              ...
                              >
                                {lang === "es" ? "🐢 Lento" : lang === "fr" ? "🐢 Lent" : lang === "pt" ? "🐢 Devagar" : "🐢 Slow"}
                              </button>
                              <button
                              ...
                              >
                                {lang === "es" ? "⚡ Rápido" : lang === "fr" ? "⚡ Rapide" : lang === "pt" ? "⚡ Rápido" : "⚡ Fast"}
                              </button>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          <button
                            type="button"
                            onClick={() => { setLangLearnMode(m => !m); setActiveWordIndex(null); }}
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${langLearnMode ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-indigo-700 border-indigo-200 hover:bg-indigo-50"}`}
                          >
                            🌍 {lang === "es" ? "Aprender Idioma" : lang === "fr" ? "Apprendre la Langue" : lang === "pt" ? "Aprender Idioma" : "Language Learning"}{langLearnMode ? " ✓" : ""}
                          </button>
                          {langLearnMode && (
                            <>
                              <span className="text-xs text-slate-500 font-medium">
                                {lang === "es" ? "Toca una palabra →" : lang === "fr" ? "Touche un mot →" : lang === "pt" ? "Toque numa palavra →" : "Tap a word →"}
                              </span>
                              <select
                                value={langLearnTarget}
                                onChange={(e) => { setLangLearnTarget(e.target.value); setActiveWordIndex(null); }}
                                className="text-xs font-bold rounded-full border border-indigo-200 bg-white text-indigo-700 px-2.5 py-1.5"
                              >
                        ...
                        <div className="prose prose-slate max-w-none">
                          {langLearnMode ? (
                            <p className="text-base sm:text-lg text-slate-700 leading-relaxed font-sans whitespace-pre-wrap">
                              {currentLesson.storySession.split(/(\s+)/).map((token, i) => {
                                if (!token.trim()) return <span key={i}>{token}</span>;
                                return (
                                  <span key={i} className="relative inline-block">
                                    <button
                                      type="button"
                                      onClick={() => handleWordTap(token, currentLesson.storySession, i)}
                                      className={`rounded px-0.5 transition-colors cursor-pointer hover:bg-amber-200 ${activeWordIndex === i ? "bg-amber-300" : ""}`}
                                    >
                                      {token}
                                    </button>
                                    {activeWordIndex === i && (
                                      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 z-30 whitespace-nowrap bg-slate-900 text-white text-sm font-bold px-2.5 py-1 rounded-lg shadow-lg">
                                        {translatingWord ? "…" : (activeWordTranslation || "—")}
                                      </span>
                                    )}
                                  </span>
                                );
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-left">
                ...
                <div className={`p-5 rounded-2xl border-2 transition-all flex flex-col justify-between relative ${
                  ...
                  <div>
                    <h4 className="text-sm font-black text-amber-600 uppercase tracking-wider">KISSS Gold</h4>
              ...
              <div className="flex items-center gap-3 mb-6 border-b border-indigo-100 pb-4">
                ...
                <div className="text-left font-sans">
                  <h3 className="text-xl font-black text-slate-900">
                    {lang === "es" ? "Centro de Gestión de Pagos y Facturación" : lang === "fr" ? "Portail de Facturation & Paiement" : "Secure KISSS Payment & Billing Portal"}
                  </h3>
                      ...
                      onSubmit={(e) => {
                        ...
                        setTimeout(() => {
                          ...
                          const newTx = {
                            ...
                            status: "Completed",
                            plan: tier === "family" ? "Supernova Family" : "KISSS Gold"
                          };
