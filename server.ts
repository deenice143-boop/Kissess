...
// 1b. Translate a single word (powers the tap-to-translate language learning feature)
app.post("/api/translate-word", async (req, res) => {
  try {
    const { word, context = "", targetLanguage = "es" } = req.body;
    ...
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      // No key configured: echo the word back so the UI still functions
      return res.json({ translation: word });
    }

    const ai = getAI();
    const prompt = `Translate the single word "${word}" into ${langName}. For context, it appears in this sentence: "${String(context).slice(0, 240)}". Reply with ONLY the ${langName} translation of that one word (one or two words maximum) — no quotes, no punctuation, no explanation.`;

    const response = await generateContentWithRetry(ai, {
      model: "gemini-3.5-flash",
      contents: prompt,
      config: {
        systemInstruction: "You are a precise bilingual dictionary for a children's learning app. Output only the single translated word or very short phrase — never a full sentence, never punctuation, never any explanation.",
      },
    });

    const translation = (response.text || word).trim().replace(/^["'.]+|["'.]+$/g, "");
    res.json({ translation: translation || word });
  } catch (err: any) {
    console.warn("[translate-word] failed:", err?.message || err);
    res.json({ translation: (req.body && req.body.word) || "" });
  }
});
...
async function run() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[KISSS] Server online at http://0.0.0.0:${PORT}`);
  });
}
